...
function usage() {
  echo "$0 [-r <repeats=$num_repeats>] [-b <start_size=$min_node_count>] [-e <end_size=$max_node_count>] [-c <bencher_subcommand>] [-d <delay>]"
}

while getopts "r:b:e:c:d:h" option; do
  case ${option} in
  r)
    num_repeats=$OPTARG
    ;;
...
function run() {
  ...
  for repeat in $(seq 1 $num_repeats); do
    run_dir="results/$binary_name-${node_count}nodes-1benchers-${subcommand}-${interval}ms/repeat${repeat}"
    ...
    if [[ ! -d $run_dir  ]]; then
        ...
        ansible-playbook main.yaml -e @values.yaml -e node_count="$node_count" -e node_image="$node_image" -e binary_name="$binary_name" -e repeat="$repeat" -e bench_type="$subcommand" -e bencher_interval="$interval"
    else
