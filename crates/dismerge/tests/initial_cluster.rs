...
async fn make_cluster(
    ...
    for (i, (client, peer, metrics)) in addresses.into_iter().enumerate() {
        ...
        tokio::spawn(async move {
            dismerge::run::<Bytes>(node_opts).await;
        });
...
async fn initial_cluster_single() {
    ...
    tokio::spawn(async move {
        dismerge::run::<Bytes>(node1_opts).await;
    });
...
async fn initial_cluster_double() {
    ...
    tokio::spawn(async move {
        dismerge::run::<Bytes>(node2_opts).await;
    });
...
async fn cluster_explicit_add() {
    for size in 1..=5 {
        test_cluster_explicit_add(size, false).await;
        tokio::time::sleep(Duration::from_millis(10)).await;
    }
}
...
async fn cluster_explicit_add_tls() {
    for size in 1..=5 {
        test_cluster_explicit_add(size, true).await;
        tokio::time::sleep(Duration::from_millis(10)).await;
    }
}
