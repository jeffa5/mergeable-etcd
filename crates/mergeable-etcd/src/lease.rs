...
impl<P: DocPersister> etcd_proto::etcdserverpb::lease_server::Lease for LeaseServer<P> {
    async fn lease_grant(
        ...
        let etcd_proto::etcdserverpb::LeaseGrantRequest { ttl, id } = request.into_inner();
    ...
    async fn lease_revoke(
        ...
        let etcd_proto::etcdserverpb::LeaseRevokeRequest { id } = request.into_inner();
    ...
    type LeaseKeepAliveStream = Pin<
        Box<
            dyn Stream<
                    Item = Result<etcd_proto::etcdserverpb::LeaseKeepAliveResponse, tonic::Status>,
                > + Send
                + Sync
                + 'static,
        >,
    ...
    async fn lease_keep_alive(
        ...
        tokio::spawn(async move {
            ...
            while let Some(Ok(request)) = request_stream.next().await {
                let etcd_proto::etcdserverpb::LeaseKeepAliveRequest { id } = request;
    ...
    {
        let etcd_proto::etcdserverpb::LeaseTimeToLiveRequest { id, keys } = request.into_inner();
    ...
    async fn lease_leases(
        ...
        let etcd_proto::etcdserverpb::LeaseLeasesRequest {} = request.into_inner();
